...
declare global { interface Window { bridge: any } }
  ...
  return (
    <div className="h-screen w-screen p-4 bg-gray-950 text-gray-100">
      <div className="flex gap-2 mb-4">
        <button onClick={()=>window.bridge.start('proxy')} className="px-3 py-2 rounded bg-blue-600 hover:bg-blue-500">Start Proxy</button>
        <button onClick={()=>window.bridge.stop('proxy')} className="px-3 py-2 rounded bg-gray-700">Stop Proxy</button>
          ...
          <tbody>
            {filtered.map((e, i) => (
              <tr key={i} className="odd:bg-gray-900/40">
                ...
                <td className="p-2">{(e.host||'') + (e.path? ` ${e.path}`:'')}</td>
                <td className="p-2">{e.event}</td>
                <td className="p-2"><pre className="whitespace-pre-wrap break-all">{JSON.stringify(e.payload, null, 0).slice(0, 500)}</pre></td>
              </tr>
            ))}
          </tbody>
        </table>
